...
int sendFile(FILE *fd)
{
	...
	while (1)
	{
		duration = (clock() * 1000) / CLOCKS_PER_SEC - t_begin;
		if (duration > TIMEOUT)
		{
		...
		else if (recvfrom(sockfd, &rcv_pkt, sizeof(rcv_pkt), MSG_DONTWAIT, (struct sockaddr *)&client_info, (socklen_t *)&len) != -1 && rcv_pkt.header.ack_num == cur_seq)
		{
			...
			cur_seq++;
			snd_pkt.header.seq_num = cur_seq;
			memset(snd_pkt.data, '\0', sizeof(snd_pkt.data));
			...
			if (remain_len > sizeof(snd_pkt.data))
				read_num = fread(snd_pkt.data, sizeof(snd_pkt.data), 1, fd);
			else
			{
